...
handler.before = async function (m) {
    ...
    if (!(id in this.susunkata)) return m.reply('Word Compose has ended')
    if (m.quoted.id == this.susunkata[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            ...
            await this.sendButton(m.chat, benar + ` +${this.susunkata[id][2]} XP`, '© ᴍᴏɴᴏʟɪᴛʜ', 'ᴀʀʀᴀɴɢᴇ ᴡᴏʀᴅs', '.susunkata', m)
