  ...
  try {
    ...
    if (/video/.test(mime)) {
      ...
      exec(`ffmpeg -ss ${args[0]} -i ${media} -to ${args[1]} -c copy ${ran}`, (err, stderr, stdout) => {
        ...
        conn.sendFile(m.chat, buff, ran, 'successfully cut video🎥', m, 0, { thumbnail: Buffer.alloc(0) })
        fs.unlinkSync(ran)
      })
    } else m.reply(`Reply video you want to cut... E.g. *${usedPrefix}cutvideo reply 00:00:01*`)
  } catch (err) {
    m.reply(`${err}`)
  }
}
